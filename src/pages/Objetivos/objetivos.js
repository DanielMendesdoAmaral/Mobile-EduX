...
const Objetivos = ({navigation}) => {
    const Item = ({nome, curso, idTurma, qtdObjetivos, navigation}) => {
        return (
            <View style={{width: "100%", height: "auto", borderWidth: 1, borderRadius: 10, borderColor: "#CCC", padding: 15, marginVertical: 10}}>
                ...
                <Text style={{marginVertical: 15, textAlign: "center"}}>Nesta turma, você possui {qtdObjetivos} objetivos.</Text>
                <View style={{width: "100%", alignItems: "center"}}>
                    <TouchableOpacity
                        style={styles.button}
                        onPress={() => navigation.navigate("ObjetivosTurma", {
                            idTurma: idTurma,
    ...
    const [objetivosDoUsuario, setObjetivosDoUsuario] = useState([]);

    useEffect(() => {
        if(turmas.length<1)
            listar();
        if(objetivosDoUsuario.length<1)
            listarUsuario();
    });
    ...
    const listarUsuario = async () => {
        try {
            const response = await fetch(`${url}/usuario/buscar/id/${idUsuarioLogado}`);
            ...
            data.alunosTurmas.map(alunoTurma => {
                obj = {
                    ...
                    qtdObjetivos : alunoTurma.objetivosAlunos.length
                }
                objetivosDoUsuario.push(obj);
            })
        }
        catch(error) {
            console.log(error);
    ...
    const renderItem = ({item}) => {
        ...
        objetivosDoUsuario.map(objetivoDoUsuario => {
            if(objetivoDoUsuario.idTurma==item.id) {
                qtdObjetivos = objetivoDoUsuario.qtdObjetivos;
            }
        });

        return <Item nome={item.descricao} curso={item.curso.titulo} idTurma={item.id} qtdObjetivos={qtdObjetivos} navigation={navigation}/>
    }
